...
resource "null_resource" "docker" {
  ...
  provisioner "local-exec" {
    ...
    command     = <<EOT
      ...
      docker tag ${local.app_name} ${aws_ecr_repository.this.repository_url}:${random_id.version.id}
      docker push ${aws_ecr_repository.this.repository_url}:${random_id.version.id}
    EOT
  }
}
